...
// BuildGetVerifyTasksRequest instantiates a HTTP request object with method
// and path set to call the "moltservice" service "get_verify_tasks" endpoint
func (c *Client) BuildGetVerifyTasksRequest(ctx context.Context, v any) (*http.Request, error) {
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: GetVerifyTasksMoltservicePath()}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("moltservice", "get_verify_tasks", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeGetVerifyTasksResponse returns a decoder for responses returned by the
// moltservice get_verify_tasks endpoint. restoreBody controls whether the
// response body should be restored after having been read.
func DecodeGetVerifyTasksResponse(decoder func(*http.Response) goahttp.Decoder, restoreBody bool) func(*http.Response) (any, error) {
	return func(resp *http.Response) (any, error) {
		if restoreBody {
			b, err := io.ReadAll(resp.Body)
			if err != nil {
		...
		case http.StatusOK:
			var (
				body GetVerifyTasksResponseBody
				err  error
			)
			err = decoder(resp).Decode(&body)
			if err != nil {
				return nil, goahttp.ErrDecodingError("moltservice", "get_verify_tasks", err)
			}
			for _, e := range body {
				if e != nil {
					if err2 := ValidateVerifyRunResponse(e); err2 != nil {
						err = goa.MergeErrors(err, err2)
					}
				}
			}
			if err != nil {
				return nil, goahttp.ErrValidationError("moltservice", "get_verify_tasks", err)
			}
			res := NewGetVerifyTasksVerifyRunOK(body)
			return res, nil
		default:
			body, _ := io.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("moltservice", "get_verify_tasks", resp.StatusCode, string(body))
		}
	}
}
...
// unmarshalVerifyRunResponseBodyToMoltserviceVerifyRun builds a value of type
// *moltservice.VerifyRun from a value of type *VerifyRunResponseBody.
func unmarshalVerifyRunResponseBodyToMoltserviceVerifyRun(v *VerifyRunResponseBody) *moltservice.VerifyRun {
	res := &moltservice.VerifyRun{
		ID:         *v.ID,
		Name:       *v.Name,
		Status:     *v.Status,
		StartedAt:  *v.StartedAt,
...
// unmarshalVerifyRunResponseToMoltserviceVerifyRun builds a value of type
// *moltservice.VerifyRun from a value of type *VerifyRunResponse.
func unmarshalVerifyRunResponseToMoltserviceVerifyRun(v *VerifyRunResponse) *moltservice.VerifyRun {
	res := &moltservice.VerifyRun{
		ID:         *v.ID,
		Name:       *v.Name,
		Status:     *v.Status,
